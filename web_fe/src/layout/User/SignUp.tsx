...
function SignUp(){
    ...
    const handleSubmit = async (event: React.FormEvent) =>{
       ...
       const checkEmail = await checkExistEmail(user.email);
        ...
        if (!user.fullname || !user.username || !user.email || !user.phonenumber || !user.province || !user.district || !user.ward || !user.password || !user.confirmpassword) {
    ...
    useEffect(() =>{
        async function fetchProvince() {
            try {
                ...
                const formattedProvince: Province[] = data.data.map((province: any) => ({
                    value: province.id,
                    label: province.name
                }));
    ...
    useEffect(() =>{
        async function fetchDistrict() {
            try {
                ...
                const formattedDistrict: District[] = data.data.map((district: any) =>({
                    value: district.id,
    ...
    useEffect(() =>{
        async function fetchWard() {
            try {
                ...
                const formattedWard: Ward[] = data.data.map((ward: any) =>({
                    value: ward.id,
        ...
        <div className="max-w-2xl md:mx-auto mx-5 bg-white shadow-2xl rounded-lg h-auto md:mt-10 mt-36 mb-10 py-5">
            ...
            <form onSubmit={handleSubmit} className="text-center mx-20">
                ...
                <div className="flex space-x-4 mb-6">
                    ...
                    <Select options={wards} className="w-1/2" placeholder="Phường/xã" onChange={handleSelectedWard} value={wards.find(w => w.value === selectedWard) || null} />
